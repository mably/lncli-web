...
module.exports = function (app, lightning, db, config) {
  ...
  app.get('/ln-sign-auth.html', (req, res) => {
    res.send('Signature verified! Authentication message was properly signed '
             + `by node ${req.userpubkey}.`);
  });

  // ln-signpayreq-auth.html
  app.get('/ln-signpayreq-auth.html', (req, res) => {
    res.send('Payment and signature verified! Authentication message was properly '
             + `signed by node ${req.userpubkey}.`);
  });

  // application -------------------------------------------------------------
  app.get('*', (req, res) => {
    // load the single view file (angular will handle the page changes on the front-end)
    res.sendFile('lnd.html', { root: `${__dirname}/../public/` });
  });
};
